...
def gridProduct():
  ...
  for i in range(0,l):
    for j in range(0,l-4):
      if(grid[i][j]*grid[i][j+1]*grid[i][j+2]*grid[i][j+3] > big):
        big = grid[i][j]*grid[i][j+1]*grid[i][j+2]*grid[i][j+3]

  #Checking up-down
  for j in range(0,l):
    for i in range(0,l-4):
      if(grid[i][j]*grid[i+1][j]*grid[i+2][j]*grid[i+3][j] > big):
        big = grid[i][j]*grid[i+1][j]*grid[i+2][j]*grid[i+3][j]

  #Checking diagonal
  for i in range(0,l-4):
    for j in range(0,l-4):
      if(grid[i][j]*grid[i+1][j+1]*grid[i+2][j+2]*grid[i+3][j+3] > big):
        big = grid[i][j]*grid[i+1][j+1]*grid[i+2][j+2]*grid[i+3][j+3]
...
print("The largest product of 4 adjacent numbers in that grid is:",gridProduct())
